...
public class CommandRemovePermission {
	...
	public CommandRemovePermission(KPermsPlugin plugin) {
		this.plugin = plugin;
	}
	public void execute(CommandSender sender, Command command, String label, String[] args) {
		if(!sender.hasPermission("kperms.user.permission.remove")) {
		...
		if(kPlayer.removePermission(args[4])) {
			kPlayer.clearPermissions();
			kPlayer.assignPermissions();
			String str = ChatColor.translateAlternateColorCodes('&', plugin.getConfigManager().getMessage("prefix") + " " + plugin.getConfigManager().getMessage("removed-permission"));
